...
<head>
    ...
    <title>Aurora Proxy for Next Dev</title>
...
<body>
    ...
    <style>
        *:not(.fas) {
        ...
        html {
            background: radial-gradient(at bottom, #111111, #000000);
        ...
        .settings-content {
            background: radial-gradient(circle at top, rgba(20, 25, 38, 0.95), rgba(5, 8, 16, 0.98));
            border-radius: 22px;
            padding: 24px 28px;
        ...
        .settings-content::before {
            ...
            border-radius: 22px;
            padding: 1px;
            background: linear-gradient(135deg, rgba(88, 200, 255, 0.35), rgba(164, 88, 255, 0.2), rgba(0, 255, 195, 0.25));
        ...
        .settings-actions button {
            ...
            text-transform: uppercase;
            box-shadow: 0 0 16px rgba(88, 200, 255, 0.2);
        }

        .settings-actions button:last-child {
            background: linear-gradient(135deg, rgba(88, 200, 255, 0.5), rgba(0, 255, 195, 0.35));
        }
    ...
    <div class="settings-panel" id="settings-panel">
        <div class="settings-content">
            <h2>Settings</h2>
            <label for="wisp-url-input">Wisp WebSocket URL</label>
            <input id="wisp-url-input" type="text" placeholder="wss://wisp.example.com/" />
    ...
    <div class="backgroundtext">
        <h1>Aurora</h1>
        <p>made by moxicat | built for Saatvik</p>
    </div>
    ...
    <script>
        ...
        const defaultWispUrl = "wss://wisp.mercurywork.shop/" // rhw wisp, super speedy
        let wispUrl = localStorage.getItem("aurora_wispUrl") || defaultWispUrl
        ...
        function closeTab(tId) {
            ...
            const frame = document.querySelector(`.viewframe[data-view-id="${tId}"]`)
            ...
            try {
                ...
                viewframe.onload = () => {
                    ...
                    if (loadC) {
                        loadC.innerHTML = `
                            <h1>ultraviolet</h1>
                            <p>made by $xor | built for nautilus-os</p>
                        `
                    }
                }
            } catch (e) {
        ...
        function saveSettings() {
            const input = document.getElementById("wisp-url-input")
            if (!input) return
            const nextUrl = input.value.trim() || defaultWispUrl
            wispUrl = nextUrl
            localStorage.setItem("aurora_wispUrl", nextUrl)
            connection.setTransport("/libcurl/index.mjs", [{ websocket: wispUrl }])
            closeSettings()
            showToast("Wisp URL updated", "fas fa-check")
        }
